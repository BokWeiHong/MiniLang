...
std::unique_ptr<ASTNode> Parser::parseProgram() {
    int indentLevel = 0;
    auto programNode = std::make_unique<ASTNode>("Start", -1);
...
std::unique_ptr<ASTNode> Parser::parseBooleanExpression(int& indentLevel) {
    ...
    if (match(TokenType::BOOLEAN_LITERAL)) {
        ...
        booleanExpressionNode->addChild(std::make_unique<ASTNode>("NOT", newIndentLevel));
        advanceToken();
        ...
        auto leftRelationalStatement = parseRelationalStatement(newIndentLevel);
        if (leftRelationalStatement != nullptr) {
            booleanExpressionNode->addChild(std::move(leftRelationalStatement));
        } else {
        ...
        if (match(TokenType::OPERATOR) && (currentToken().getValue() == "AND" || currentToken().getValue() == "OR")) {
            ...
            booleanExpressionNode->addChild(std::make_unique<ASTNode>(operatorValue, newIndentLevel));
            advanceToken();
            auto rightRelationalStatement = parseRelationalStatement(newIndentLevel);
            if (rightRelationalStatement != nullptr) {
                booleanExpressionNode->addChild(std::move(rightRelationalStatement));
                return booleanExpressionNode;
            } else {
...
std::unique_ptr<ASTNode> Parser::parseExpression(int& indentLevel) {
    ...
    if (match(TokenType::OPERATOR) && (currentToken().getValue() == "+" || currentToken().getValue() == "-" 
        ...
        std::string operatorValue = currentToken().getValue();
        expressionNode->addChild(std::make_unique<ASTNode>(operatorValue, newIndentLevel));
        advanceToken();
        auto expression2 = parseExpression(newIndentLevel);
        if (expression2 != nullptr) {
